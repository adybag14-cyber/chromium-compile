...
- Source cache contract `chromium-src-v3-<version>` stores the trust marker plus SHA-bound bounded archive stats (member count and declared unpacked bytes), all tied to version + GCS generation/length/MD5 + SHA-256 + successful safe-archive/Gitiles proofs. v2 and older caches are accepted only as migration fallbacks, fully revalidated, then promoted to v3. Exact v3 bytes can skip redundant decompression/Gitiles scans on later stages while still rechecking the complete compressed object against GCS metadata and proving extraction capacity from the trusted stats.
