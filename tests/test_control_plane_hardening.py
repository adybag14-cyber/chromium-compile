...
class ControlPlaneHardeningTests(unittest.TestCase):
    ...
    def test_bootstrap_is_manual_release_guarded_and_exactly_once(self):
        ...
        self.assertIn('WORKFLOW_SHA: ${{ github.workflow_sha }}', bootstrap)
        self.assertIn('--expected-head-sha "${WORKFLOW_SHA}"', bootstrap)
    ...
    def test_watcher_state_queries_are_bounded_and_include_publisher(self):
        ...
        self.assertIn("--expected-head-sha", (ROOT / ".github" / "workflows" / "watch-chromium-stable.yml").read_text(encoding="utf-8"))
