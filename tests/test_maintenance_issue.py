...
class MaintenanceIssueTests(unittest.TestCase):
    ...
    def test_duplicate_exact_titles_fail_closed_without_retry(self):
        payload = json.dumps([
            {"number": 1, "title": "target"},
        ...
        with mock.patch.object(
            ...
            "run_gh",
            return_value=subprocess.CompletedProcess(["gh"], 0, payload, ""),
        ) as run_gh, mock.patch.object(issues.time, "sleep") as sleep:
            with self.assertRaises(issues.IssueStateError):
                issues.find_issue("owner/repo", "target", attempts=3)
