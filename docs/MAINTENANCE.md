...
1. Verify the requested `chromium-<version>.tar.xz` against GCS generation, length, MD5 and local SHA-256; stream-check paths/resource bounds before extraction; compare Windows-critical files with the exact Gitiles tag. Resolve that same tag's exact 40-character commit, canonical `Cr-Commit-Position`, and UTC committer time from Gitiles. Materialize those values as `build/util/LASTCHANGE` and `LASTCHANGE.committime`, then execute the revision's own `compute_build_timestamp.py default` before GN. The resulting Windows PE/COFF timestamp must be positive, bounded, not in the future, and no newer than the tag commit; never restore the zero-time dummy fallback.
...
6. Parse the first-class Windows clang-format, Windows Node, Node modules, Rust toolchain and libclang GCS objects plus the Windows TypeScript CIPD package and Windows-only gperf, DirectX-Headers, WebAuthn and Perl Git revisions from root `DEPS`; parse the unconditional esbuild and rollup CIPD packages from the nested DevTools `DEPS`; and parse the Windows Go package plus its immutable version variable from the nested Dawn `DEPS`, without evaluating any DEPS file. Download each generation-qualified GCS object only from its fixed Chromium bucket, enforce its declared byte length and SHA-256, reject unsafe or Windows-ambiguous archive members, install only immutable source-declared CIPD package/version pairs, and fetch Git dependencies only from their fixed Chromium mirrors at exact 40-character revisions. Run the pinned DevTools `sync_rollup_libs.py` hook to materialize the platform package under `node_modules/@rollup`, then execute the real Rollup CLI as a runtime probe. Validate clang-format, Node, the Node modules structure, `bindgen.exe`, Cargo, rustc, rustfmt, libclang, `tsc.exe`, esbuild, Rollup, Dawn Go, gperf, Perl, DirectX/WebAuthn headers, and Chromium's own Rust revision stamp. Carry canonical GCS, CIPD and Git descriptor SHA-256 values through prepared state, checkpoints, evidence and the release manifest.
7. Generate the graph with the small cross-revision GN contract in `scripts/chromium_windows_pipeline.py` and query both `//chrome` and `//chrome/installer/mini_installer:mini_installer`. Resolve `//chrome`'s bounded direct executable dependency set through GN, identify exactly one target whose computed output is `chrome.exe`, and require that target's resolved linker flags to contain only the `/TIMESTAMP` value already returned by Chromium's calculator. This deliberately avoids depending on Chromium's internal executable target name or scanning the multi-gigabyte generated graph. Then perform a quiet full Ninja dry-run of `chrome` and `mini_installer` to traverse the generated input closure. Because Ninja dry-runs do not launch host tools, compatibility preflight must also execute Dawn's real `//third_party/dawn/src/tint:generate_sources` action with `GOTOOLCHAIN=local` and verify every GN-declared output. The executable probes, real Dawn generation, linker inspection, and graph dry-run together turn absent source-declared tools or invalid Windows linker metadata into a preflight failure instead of a production Stage 1 failure.
...
The producer run must be the exact Windows build workflow in the same repository, on the expected branch and immutable lineage SHA, with a version/stage title matching the requested producer. The manifest additionally binds source SHA, authoritative Chromium tag commit/position/time, the validated Windows linker timestamp, Chromium-pinned depot_tools/GN/Ninja/Clang, SDK family, VS year, the fixed resume-input epoch, port hash, run ID/attempt, archive length and archive SHA. As in the proven Linux resume lane, every immutable non-output source/tool file, symlink and directory is normalized to the fixed epoch before checkpoint restoration. GN also records Visual Studio, Windows Kits 10, and Windows Kits NETFXSDK directory mtimes in `build.ninja.d`; those exact-version directories are recreated on every fresh hosted runner. The resume path therefore validates every depfile input against the exact source and resolved VS/SDK roots, then advances only the zero-byte `build.ninja.stamp` beyond those validated mtimes. It does not regenerate `build.ninja`, alter `.ninja_log`/`.ninja_deps`, or touch a compiled output. A bounded Ninja manifest probe must still report the restored graph clean, so any other regeneration cause fails closed. GN is regenerated only when the checkpoint's SDK servicing, Visual Studio product version, or runner image version differs from the current runner, because that is genuine toolchain drift.

The manifest-stamp repair has one repository-coded immutable bridge for Stage 1 run `33274094424`. It is bound to Chromium `153.0.8010.12`, producer SHA `70443e888304bc1ac17e986e33f1fab605243fca`, stage 1, the prior port-configuration digest, and archive SHA-256 `6f124bee59ed5693db7a0477f91ad57330f990cc0f32aa43fe0e9cabb426e058`. This preserves the 48,768 already-recorded outputs without creating a generic cross-lineage exception. The first successor checkpoint is written with the current lineage and configuration and returns to ordinary strict validation.

Windows no-progress accounting compares the unique normalized output paths in `.ninja_log`, not its raw line count. A multi-output edge and a rebuilt output can append multiple raw rows without adding any durable output; those rows remain visible for diagnostics but cannot reset the two-slice no-progress streak. Stage summaries report both raw rows and unique outputs.
