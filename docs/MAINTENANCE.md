...
6. Parse the first-class Windows clang-format, Windows Node, Node modules, Rust toolchain and libclang GCS objects plus the Windows TypeScript CIPD package from that exact `DEPS` without evaluating the file. Download each generation-qualified GCS object only from its fixed Chromium bucket, enforce its declared byte length and SHA-256, reject unsafe or Windows-ambiguous archive members, and install only the immutable source-declared CIPD package/version. Validate clang-format, Node, the Node modules structure, `bindgen.exe`, Cargo, rustc, rustfmt, libclang, `tsc.exe`, and Chromium's own Rust revision stamp. Carry canonical GCS and CIPD descriptor SHA-256 values through prepared state, checkpoints, evidence and the release manifest.
