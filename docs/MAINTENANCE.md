...
7. Generate the graph with the small cross-revision GN contract in `scripts/chromium_windows_pipeline.py`, query both `//chrome` and `//chrome/installer/mini_installer:mini_installer`, and scan every bounded regular generated `.ninja` file to require that every `/TIMESTAMP` occurrence equals the value already returned by Chromium's calculator. This deliberately avoids depending on Chromium's internal executable target name. Then perform a quiet full Ninja dry-run of `chrome` and `mini_installer`. This traverses the generated input closure and turns missing source-declared host tools or invalid Windows linker metadata into a preflight failure instead of a production Stage 1 failure.
