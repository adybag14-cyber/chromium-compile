...
6. Parse the first-class `src/third_party/rust-toolchain` and `src/third_party/llvm-libclang` Windows GCS objects from that exact `DEPS` without evaluating the file. Download the generation-qualified objects only from the fixed Chromium bucket, enforce the declared byte length and SHA-256, reject unsafe or Windows-ambiguous archive members, then validate `bindgen.exe`, Cargo, rustc, rustfmt, libclang and Chromium's own Rust revision stamp. Carry the canonical two-object descriptor SHA-256 through prepared state, checkpoints, evidence and the release manifest.
7. Generate the graph with the small cross-revision GN contract in `scripts/chromium_windows_pipeline.py`, then query both `//chrome` and `//chrome/installer/mini_installer:mini_installer` before compilation.
