...
6. Parse the first-class Windows clang-format, Windows Node, Node modules, Rust toolchain and libclang GCS objects from that exact `DEPS` without evaluating the file. Download each generation-qualified object only from its fixed Chromium bucket, enforce the declared byte length and SHA-256, reject unsafe or Windows-ambiguous archive members, then validate clang-format, Node, the Node modules structure, `bindgen.exe`, Cargo, rustc, rustfmt, libclang and Chromium's own Rust revision stamp. Carry the canonical five-object descriptor SHA-256 through prepared state, checkpoints, evidence and the release manifest.
